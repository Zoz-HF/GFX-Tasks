...
#include <cstdint>  // Include this header for intptr_t

static intptr_t font = reinterpret_cast<intptr_t>(GLUT_BITMAP_8_BY_13);
    ...
    if (spaceBarCounter == 2 && (!(checkCollisionTarget() || collisionBlocks))) {
        switch (key) {
            ...
            case GLUT_KEY_PAGE_DOWN: {
                cout << "key page down, the spot angle is " << spotAngle << endl;
                if (spotAngle > 0.0) spotAngle -= 1.0;
            }
                break;
            case GLUT_KEY_PAGE_UP: {
                cout << "key page up, the spot angle is " << spotAngle << endl;
...
void keyInput(unsigned char key, int x, int y) {
    ...
    switch (key) {
        ...
        case ' ':
            ...
            break;
        case 't': {
            cout << "key t, the spot exponent is " << spotExponent << endl;
        ...
        case 's':
            ...
            cout << "show hide cone " << showHideCone << endl;
            break;
        case 'p':
            light1On = !light1On;
            cout << " light 1 is " << light1On << endl;
            break;
        case 'a':
            light2On = !light2On;
            cout << " light 2 is " << light1On << endl;
            break;
        case 'q':
            angle += 5.0;
            cout<<angle;
...
void drawScene(void) {
    ...
    if (spaceBarCounter == 2) {
        ...
        if (spaceBarCounter == 0) {
            ...
            for (char c: "press space bar to continue, p to turn the spotlight on and off,press space bar to continue ") {
                glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, c);
            }
    ...
    cout << "L1 L2 L3 are " << L1 << " " << L2 << " " << L3 << endl;
